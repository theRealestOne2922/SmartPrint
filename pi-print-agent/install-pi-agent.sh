...
async function processJob(job) {
    ...
    try {
        ...
        let lpCommand = `lp -d SmartPrint -n ${copies}`;
        if (job.color_mode === 'bw') lpCommand += ` -o ColorModel=Gray`;
        else lpCommand += ` -o ColorModel=RGB`;
...
async function cleanupOldJobs() {
    try {
        let hoursToKeep = parseFloat(process.env.CLEANUP_HOURS) || 24;
        const { data: settings } = await supabase.from('admin_settings').select('key, value').eq('key', 'jobExpirationHours').single();
        if (settings && settings.value) {
            hoursToKeep = parseFloat(settings.value);
            console.log(`[CLEANUP] Using admin portal retention setting: ${hoursToKeep} hours`);
        }
        ...
        for (const job of oldJobs) {
            try {
                if (job.file_path && job.file_path.includes('/storage/v1/object/public/print_jobs/')) {
                    const storagePath = job.file_path.split('/storage/v1/object/public/print_jobs/')[1];
                    if (storagePath) {
                        await supabase.storage.from('print_jobs').remove([storagePath]);
                    }
                }
        ...
        console.log(`[CLEANUP] ✅ Successfully deleted ${deletedCount} old files.`);
    } catch (err) {
...
echo "🔑 Pre-configuring Supabase environment keys..."
cat << 'EOF' > "$INSTALL_DIR/.env"
SUPABASE_URL="https://mqqluwvemcuokqcchnii.supabase.co"
SUPABASE_SERVICE_ROLE_KEY="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im1xcWx1d3ZlbWN1b2txY2NobmlpIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc3ODkyNTAzNCwiZXhwIjoyMDk0NTAxMDM0fQ.LeKWAc38zSu13_sjllAtbd6kbWBJyk146JvDc9Dcu3w"
CLEANUP_HOURS=24
EOF
